...
```mermaid
flowchart LR
    A[Discover a project] --> B[Read README and contribution guidance]
    B --> C[Choose issue or discussion]
    C --> D[Fork or create a branch]
    D --> E[Open a pull request]
    E --> F[Maintain respectful collaboration]
    G[InnerSource] -. applies similar practices inside an organization .-> B
```
